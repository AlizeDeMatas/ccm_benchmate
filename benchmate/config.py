...
# TODO this need to be passed to literature
literature = {"lp_model": {"model_path": f"{resource_path}/lp_model/model_final.pth",
...
inference = {"vl_model": {"model": {"name": "Qwen/Qwen2.5-VL-3B-Instruct", "max_tokens": 100,
                                    "config": {"cache_dir": f"{resource_path}/hf_models", }, },
                          "processor": {"name": "Qwen/Qwen2.5-VL-3B-Instruct",
                                        "config": {"cache_dir": f"{resource_path}/hf_models"}}
                          },

             "text_embedding_model": {"name": "Qwen/Qwen3-Embedding-0.6B",
                 "config": {"cache_folder": f"{resource_path}/hf_models"}}, "image_embedding_model": {
        "model": {"name": "vidore/colpali-v1.3", "config": {"cache_dir": f"{resource_path}/hf_models"}},
        "processor": {"name": "vidore/colpali-v1.3", "config": {"cache_dir": f"{resource_path}/hf_models"}}},
             "chunker_model": {"model": f"{resource_path}/m2v_model/", "min_sentences": 1, "return_type": "texts",
                               "threshold": "auto", "chunk_size": 100, }, }

api_call = {"text_embedding_model": {"model": f"{resource_path}/m2v_model/"}}

# TODO
search = {"text_embedding_model": literature["text_embedding_model"],
          "api_call_embedding_model": api_call["text_embedding_model"]["model"],
          "image_embedding_model": literature["image_embedding_model"], "causal": {
        "model": {"name": "chose a name", "model": f"{resource_path}/hf_models",  # TODO get kv cache options,
        }, "tokenizer": {"name": "choose a name", "model": f"{resource_path}/hf_models", "truncation": True
            # TODO other options
        }, "quantization": {  # TODO
